...
This project involves two steps: (1) Build a RocksetSpecGPT to create custom OpenAPI spec and (2) Build a RocksetGPT with those specs. Optionally, you can skip part 1 and use the specs included in this repository to build RocksetGPT.
...
- Network Issue usually means there is an issue with OpenAI's client code connecting to Rockset's API. Can be a result from incorrectly setting up your Authentication and/or incorrect schema
...
- Since you have to explictly define the authentication method, avoid including security schemas in the OpenAPI spec as this can sometimes cause issues. OpenAI's client code will automatically create the security schema internally.
